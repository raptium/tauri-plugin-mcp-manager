...
async function startMcpServer(payload) {
    return await core.invoke(`plugin:${PLUGIN_NAME}|start_mcp_server`, { payload });
}
async function sendToMcpServer(payload) {
    await core.invoke(`plugin:${PLUGIN_NAME}|send_to_mcp_server`, { payload });
}
async function killMcpServer(payload) {
    await core.invoke(`plugin:${PLUGIN_NAME}|kill_mcp_server`, { payload });
}
// --- Event Listening ---
/**
...
async function listenToMcpServer(serverId, onMessage) {
    ...
    return await event.listen(eventName, (event) => {
        onMessage(event.payload);
    });
}

function deserializeMessage(line) {
    return types_js.JSONRPCMessageSchema.parse(JSON.parse(line));
}
...
exports.TauriStdioTransport = TauriStdioTransport;
